export const state = {
  rawEvents: [],
  filters: { range: "7d", category: "all", minConfidence: 50 }
};
export function setRawEvents(list) { state.rawEvents = list; }
export function setFilter(key, val) { state.filters[key] = val; return state.filters; }
export function getFilteredEvents(now = new Date()) {
  ...
  return state.rawEvents.filter(e => {
    ...
    return (!isNaN(t) && t >= minTs) && catOk && confOk;
  });
}
